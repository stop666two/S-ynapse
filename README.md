...
**全配置驱动**
- 13 个 JSON5 配置文件（支持注释），**2000+ 可配置项**（实测 2413 项），逐字段中文注释（含可填值/推荐值/禁用值/注意事项）
- `features.json5` 功能总控域：**95 个模块、765 个配置项**，每项功能均可开/关/微调；`tuning.json5` UI 微调层（31 分类 / 202 项）
...
├── features.json5     # 功能总控（95 模块/765 项，可开关/微调，可选文件）
...
`features.json5` 是全部交互与内容功能的统一开关域：95 个模块、765 个配置项，逐项中文注释。几例：

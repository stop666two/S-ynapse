...
**全配置驱动**
- 13 个 JSON5 配置文件（支持注释），**2000+ 可配置项**（实测 2347 项），逐字段中文注释（含可填值/推荐值/禁用值/注意事项）
- `features.json5` 功能总控域：**92 个模块、742 个配置项**，每项功能均可开/关/微调；`tuning.json5` UI 微调层（30 分类 / 196 项）
...
| `guard.json5` | 防护与交互控制域（137 项：自定义右键菜单、复制控制/署名、选择控制、快捷键拦截、水印、检测与控制台反制、窗口隐私帘、绕过通道等） | 可选（缺失时防护功能关闭） |

...
**全配置驱动**
- 12 个 JSON5 配置文件（支持注释），**2000+ 可配置项**（实测 2080 项），逐字段中文注释（含可填值/推荐值/禁用值/注意事项）
- `features.json5` 功能总控域：**86 个模块、694 个配置项**，每项功能均可开/关/微调；`tuning.json5` UI 微调层（28 分类 / 185 项）
...
├── features.json5     # 功能总控（86 模块/694 项，可开关/微调，可选文件）
...
`features.json5` 是全部交互与内容功能的统一开关域：86 个模块、694 个配置项，逐项中文注释。几例：
...
| 步骤 | 操作 | 说明 |
|------|------|------|
| 1 | 加载配置 | 12 个 JSON5 配置（含 tuning.json5）+ 可选 content-policy.json5/tag-aliases.json5/friends.json5，合并默认值，语法错误即终止（报告文件/行列/原因），20+ 项值域校验 + features 86 模块结构校验 |

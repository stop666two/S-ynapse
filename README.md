...
- `features.json5` 功能总控域：**80 个模块、631 个配置项**，每项功能均可开/关/微调；`tuning.json5` UI 微调层（27 分类 / 179 项）
...
├── features.json5     # 功能总控（80 模块/631 项，可开关/微调，可选文件）
...
| `features.json5` | 80 个功能模块的开关/参数（灯箱、进度条、快捷键、公式、分享、预设、定时、收藏、评论…） | 可选（缺失回退默认，功能保持） |
...
`features.json5` 是全部交互与内容功能的统一开关域：80 个模块、631 个配置项，逐项中文注释。几例：
...
| 步骤 | 操作 | 说明 |
|------|------|------|
| 1 | 加载配置 | 12 个 JSON5 配置（含 tuning.json5）+ 可选 content-policy.json5/tag-aliases.json5/friends.json5，合并默认值，语法错误即终止（报告文件/行列/原因），20+ 项值域校验 + features 80 模块结构校验 |

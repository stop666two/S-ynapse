...
**全配置驱动**
- 13 个 JSON5 配置文件（支持注释），**2000+ 可配置项**（实测 2403 项），逐字段中文注释（含可填值/推荐值/禁用值/注意事项）
- `features.json5` 功能总控域：**94 个模块、755 个配置项**，每项功能均可开/关/微调；`tuning.json5` UI 微调层（31 分类 / 202 项）
...
├── features.json5     # 功能总控（94 模块/755 项，可开关/微调，可选文件）
├── ui-strings.json5   # 界面文案词典（zh/en 双语词典，服务端 ui() + 运行时 __T()，可选）
├── tuning.json5       # UI 微调参数层（31 分类/202 项，注入 CSS 变量；行为参数运行时读取，可选）
├── guard.json5        # 防护与交互控制域（164 项：右键/复制/选择/快捷键/水印/检测/控制台/隐私帘/篡改监视/访问门槛，逐项注释，可选）
...
| `features.json5` | 94 个功能模块的开关/参数（灯箱、进度条、快捷键、公式、分享、预设、定时、收藏、评论…） | 可选（缺失回退默认，功能保持） |
| `ui-strings.json5` | 界面文案词典（zh/en 双语，i18n 切换的文案来源） | 可选（缺失回退内置文案） |
| `tuning.json5` | UI 微调参数层（31 分类 / 202 项：排版/间距/圆角/动效/组件细节，注入 CSS 变量） | 可选 |
| `guard.json5` | 防护与交互控制域（164 项：自定义右键菜单、复制控制/署名、选择控制、快捷键拦截、水印、检测与控制台反制、窗口隐私帘、篡改监视、访问门槛、绕过通道等） | 可选（缺失时防护功能关闭） |
...
`features.json5` 是全部交互与内容功能的统一开关域：94 个模块、755 个配置项，逐项中文注释。几例：

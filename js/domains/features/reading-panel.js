export function init() {
  (function () {
    ...
    function save() { try { localStorage.setItem(PK, JSON.stringify(prefs)); } catch (e) { /* 忽略：存储不可用时偏好仅当次会话有效 */ } }
    var rset = document.getElementById('rReset');
    if (rset) rset.onclick = function () { prefs = {}; try { localStorage.removeItem(PK); } catch (e) { /* 忽略：存储不可用时偏好仅当次会话有效 */ } apply(); syncUI(); };
    apply();
  })();
}

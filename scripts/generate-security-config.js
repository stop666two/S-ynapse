...
// ==================== 双配置漂移消除 ====================
// security.json 是唯一配置源。本模块在每次构建时从 security.json 提取
...
function renderWorkerConfig(extracted) {
  ...
  return (
    ...
    '// Single source of truth: security.json. Regenerate with `npm run build`.\n' +
    'export default ' + body + ';\n'
  );
}

/** 读 security.json → 写 workers/security-config.js。返回生成的文件路径。 */
...
if (require.main === module) {
  ...
  const securityPath = process.argv[2] || path.join(ROOT, 'security.json');
  if (!fs.existsSync(securityPath)) {
    console.error(`[FATAL] security.json not found at ${securityPath}`);
    process.exit(1);
  }
